...
- **`link_type="button"` is now a declared value on `tab`.** `Link.a` always rendered it (the playground has used button tabs since the tabs page shipped), but the attr declaration didn't admit it, so every compile emitted value warnings. Declared and documented: a real `<button>` for phx-click / JS-command tabs.
