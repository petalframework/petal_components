...
- **Radio-card selection calibrates for dark mode.** The checked border used `primary-600` in both schemes; on a monochrome primary, dark-mode 600 resolves near-white, framing the selected card in a glaring outline. Dark now uses `primary-500` - the restrained selected border on monochrome (the REUI school), still vivid on hue themes. Light is unchanged, and the selection dot keeps its high-contrast ring.
