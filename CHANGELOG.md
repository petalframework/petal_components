...
- **`angle` and `color` on meteors.** The meteor shower's direction and colour were hardcoded; both are attrs now (defaults unchanged), so a field can streak any way in any colour.
