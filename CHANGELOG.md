...
- **`art` on `avatar` - generative art placeholders.** `art="mesh"` draws a soft multi-hue gradient orb (layered radial blobs at hashed positions), `art="dither"` a two-tone ordered-dither blend with solid corners and a chunky pixel band across the diagonal - the boringavatars school, natively. Deterministic per name, pure CSS plus a ~1KB inline SVG, no JavaScript, no dependencies. Pure art by default; add `initials` to overlay the monogram - because the palette is generated server-side, the text goes dark in the base hue, and the dither switches to a badge form - a large solid centre with a sparse, symmetric pixel ring around the rim on a chunkier grid - so the monogram sits on solid colour, legible without a scrim. The name always labels the avatar for screen readers, and a photo `src` wins when present.
